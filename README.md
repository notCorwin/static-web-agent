...
- Text-bearing PDFs and ordinary DOCX, PPTX, XLSX, CSV, and text files are converted to Markdown by anydoc in a dedicated Worker. PDFs that produce no meaningful text fall back to PDF.js page rendering in its dedicated Worker, yielding one page at a time and then using the vision/OCR route above, preserving page order without retaining a second full-document page array.
